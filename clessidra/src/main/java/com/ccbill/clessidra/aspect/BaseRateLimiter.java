    ...
    public Object intercept(ProceedingJoinPoint joinPoint)
        ...
        if (rateLimitedAnnotation != null) {
            ...
            synchronized (BaseRateLimiter.class) {
                ...
                if (queue.peek() != null) {
                    if (queue.size() < rateLimitedQueueAnnotation.queueSize()) {
                        queue.offer(Thread.currentThread());
                        logger.debug("Put thread " + Thread.currentThread().getName() + " in queue");
                        sleepEternally = true;
                    }
                    else {
            ...
            if (sleepEternally) {
                ...
                logger.debug("Awakened thread " + Thread.currentThread().getName() + " from eternal sleep.");
            }
